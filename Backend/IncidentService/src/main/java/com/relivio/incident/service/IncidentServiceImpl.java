...
public class IncidentServiceImpl implements IncidentService {
    ...
    private static final Set<String> SAFE_TO_PATCH_FIELDS = Set.of(
            "title", "description", "location", "severity", "disasterType", "status",
            "reporterId", "reporterName", "reporterContact"
    );

    private static final Map<IncidentStatus, Set<IncidentStatus>> ALLOWED_TRANSITIONS = Map.of(
            IncidentStatus.REPORTED, EnumSet.of(IncidentStatus.VERIFIED),
    ...
    public IncidentResponse patchIncident(Long incidentId, Map<String, Object> updates) {
        ...
        for (Map.Entry<String, Object> entry : updates.entrySet()) {
            ...
            if (!SAFE_TO_PATCH_FIELDS.contains(key)) {
                log.warn("Attempted to patch a forbidden field: {} for incident ID: {}", key, incidentId);
