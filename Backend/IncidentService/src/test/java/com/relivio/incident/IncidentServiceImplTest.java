...
public class IncidentServiceImplTest {
    ...
    @Test
    void testUpdateIncident_reportedToClosed_shouldThrow() {
        when(incidentRepository.findById(1L)).thenReturn(Optional.of(incident));
        incidentRequest.setStatus(IncidentStatus.CLOSED);
    ...
    @Test
    void testUpdateIncident_reportedToResolved_shouldThrow() {
        when(incidentRepository.findById(1L)).thenReturn(Optional.of(incident));
    ...
    @Test
    void testPatchIncident_reportedToClosed_shouldThrow() {
        when(incidentRepository.findById(1L)).thenReturn(Optional.of(incident));

        assertThrows(InvalidIncidentStateException.class,
                () -> incidentService.patchIncident(1L, Map.of("status", "CLOSED")));
        verify(incidentRepository, never()).save(any(Incident.class));
    }
