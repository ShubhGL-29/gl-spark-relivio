...
public class ResourceServiceImplTest {
    ...
    @Test
    void searchResources_shouldFilterByName() {
        when(resourceRepository.findByResourceNameContainingIgnoreCase(eq("water"), any()))
                .thenReturn(new PageImpl<>(List.of(resource)));

        Page<ResourceResponse> result = resourceService.searchResources("water", PageRequest.of(0, 10));

        assertNotNull(result);
        assertEquals(1, result.getTotalElements());
        verify(resourceRepository).findByResourceNameContainingIgnoreCase("water", PageRequest.of(0, 10));
    }

    @Test
    void searchResources_shouldReturnAll_whenNameBlank() {
        PageRequest pageRequest = PageRequest.of(0, 10);
        when(resourceRepository.findAll(pageRequest)).thenReturn(new PageImpl<>(List.of(resource)));

        Page<ResourceResponse> result = resourceService.searchResources(" ", pageRequest);

        assertNotNull(result);
        assertEquals(1, result.getTotalElements());
        verify(resourceRepository).findAll(pageRequest);
    }
